...
test("GET / sert la page d’accueil en HTML", async () => {
  ...
  assert.ok(
    corps.includes("<main"),
    "la page d’accueil doit contenir un repère <main>",
  );
});
...
test("GET /styles.css sert la feuille de style en CSS", async () => {
  ...
  assert.ok(
    corps.trim().length > 0,
    "la feuille de style ne doit pas être vide",
  );
});
...
test("les chemins privés ne divulguent aucun fichier", async () => {
  ...
  for (const chemin of chemins) {
    ...
    assert.ok(
      !mime.includes("javascript"),
      `MIME neutre attendu pour ${chemin}, reçu : ${mime}`,
    );
  }
});
