...
test.describe('Identité Archia — accueil (critère 3)', () => {
  test('la conversation vide affiche l’accueil exact, hors de #messages', async ({ page }) => {
    await pageNeuve(page);
...
test.describe('Identité Archia — suggestions (critère 4)', () => {
  test('la page propose exactement les trois questions suggérées, hors de #messages', async ({ page }) => {
    await pageNeuve(page);
...
test.describe('Identité Archia — réponses signées (critère 5)', () => {
  test('la ligne de l’assistant commence par « Archia : »', async ({ page }) => {
    await pageNeuve(page);
    await envoyer(page, 'salut');
    await expect(lignes(page)).toHaveCount(2);
    const texte = await lignes(page).nth(1).evaluate((el) => el.textContent ?? '');
